...
| 모드 | 비율 | 저장 해상도 (5s·5s+·6·OFF) | 저장 해상도 (6s) |
|---|---|---|---|
| 5:5 (기본) | 1:1 정사각 | 2448 × 2448 | 3024 × 3024 |
| 4:3 | 세로 3:4 | 2448 × 3264 | 3024 × 4032 |

두 비율 모두 4:3 센서에서 나오며, 5:5 는 가운데를 잘라 씁니다.
저장 해상도는 흉내내는 기종의 센서에 맞춰 다운스케일합니다 — 5s·6 은 8MP(3264×2448), 6s 는 12MP(4032×3024).
...
| 상단 우측 | 3분할 그리드, 수평계 |
| 하단 칩 | 비율 전환 (5:5 ↔ 4:3), 룩 전환 (5s → 5s+ → 6 → 6s → OFF) |
| 좌측 하단 썸네일 | 사진 앱 열기 |
...
| 6 | `iPhone6` | iPhone 6 (2014). 같은 8MP 지만 색이 중립에 가깝고 하이라이트가 덜 날아감. 노이즈는 줄고 뭉개짐이 늘어남 |
| 6s | `iPhone6s` | iPhone 6s (2015). 12MP 로 디테일·샤프닝이 늘고 색이 더 진하고 따뜻함. 알갱이가 더 잘음 |
| OFF | `off` | 룩 없음 (원본) |

5s → 6 → 6s 로 갈수록 색은 따뜻·진해지고 다이나믹 레인지는 넓어지는 방향으로 잡았습니다.
노이즈는 6 이 가장 적고(노이즈 리덕션 강화), 6s 는 픽셀이 작아져 조금 늘지만 알갱이가 더 잘습니다.
